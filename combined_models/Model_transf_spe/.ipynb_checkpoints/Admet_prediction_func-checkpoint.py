...
from transformer.Model.training_funcs import create_masks
from combined_models.Model_transf_spe.training_funcs import CombinedTransfBert, SMILESDatasetComb
...
def train_model(model, train_loader, valid_loader, criterion, optimizer, 
    ...
    for epoch in range(num_epochs):
        ...
        for batch, (X, Y) in tqdm.tqdm(enumerate(train_loader)):
            ...
            mask = create_masks(X2)
            pred, embed = model(X1, X2, mask)
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), f"{model_dir}/best_tbcombined_model.pt")
        else:
            patience_counter += 1
...
def admet_performance(seed_options, tokenizer, spe, seq_len, d_model, n_layers, heads, dropout, device, 
                      lr, batch_size, num_epochs, group, group_config, model_dir, alphabet_size,
                      hidden_layer = 256, blocks_freeze_t = ['module.encoder.layers.5.', 
    ...
    for seed in seed_options:
        ...
        for benchmark in group:
            ...
            train_dataset = SMILESDatasetComb(data=train, tokenizer = tokenizer, spe = spe, 
                                              seq_len = seq_len, mean=mean, sd=sd,
                                              task_type=task_type, change_smi=True)
            valid_dataset = SMILESDatasetComb(data=valid, tokenizer = tokenizer, spe = spe, 
                                              seq_len = seq_len, mean=mean, sd=sd,
                                              task_type=task_type, change_smi=False)
            test_dataset = SMILESDatasetComb(data=test, tokenizer = tokenizer, spe = spe, 
                                              seq_len = seq_len, mean=mean, sd=sd,
                                              task_type=task_type, change_smi=False)
            ...
            for name, param in transformer.named_parameters():
                if 'module.encoder.' in name and not any(block in name for block in blocks_freeze_t):
                    param.requires_grad = False
                
            transformer.to(device)
            
            admet_prediction = CombinedTransfBert(hidden = hidden_layer,
                                                  bert_model = bert_model, 
                                                  transformer = transformer, 
                                                  task_type = task_type, 
                                                  output = 1, dropout = dropout)
            ...
            admet_prediction.load_state_dict(torch.load(f"{model_dir}/best_tbcombined_model.pt", weights_only=True))
            ...
            with torch.no_grad():
                for batch, (X, Y) in tqdm.tqdm(enumerate(test_dataloader)):
                    ...
                    mask = create_masks(X2)
                    pred, embed = admet_prediction(X1, X2, mask)
